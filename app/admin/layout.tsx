...
function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto grid w-full max-w-7xl flex-1 gap-6 px-4 py-6 md:grid-cols-[240px_1fr] md:px-6">
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          {org ? (
            <div className="mb-3 rounded-xl bg-slate-50 px-3 py-2 ring-1 ring-slate-200">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Organisation</p>
              <p className="mt-1 truncate text-sm font-semibold text-slate-800">{org.name}</p>
            </div>
          ) : null}
            ...
            {isOrgAdmin ? (
              <>
                ...
                {orgOpen ? (
                  <div className="grid gap-1 pl-4">
                    <Link
                      href="/admin/organisation/einstellungen"
                      className={`rounded-lg px-3 py-2 text-sm transition ${
                        pathname === '/admin/organisation/einstellungen'
                          ? 'bg-emerald-100 text-emerald-900 font-medium'
                          : 'text-slate-700 hover:bg-slate-100'
                      }`}
                    >
                    ...
                    <Link
                      ...
                      className={`rounded-lg px-3 py-2 text-sm transition ${
                        pathname === '/admin/organisation/mitglieder'
                          ? 'bg-emerald-100 text-emerald-900 font-medium'
                          : 'text-slate-700 hover:bg-slate-100'
                      }`}
                    >
                    ...
                    <Link
                      ...
                      className={`rounded-lg px-3 py-2 text-sm transition ${
                        pathname === '/admin/organisation/events'
                          ? 'bg-emerald-100 text-emerald-900 font-medium'
                          : 'text-slate-700 hover:bg-slate-100'
                      }`}
                    >
